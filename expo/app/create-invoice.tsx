...
export default function CreateInvoiceScreen() {
  ...
  const [businessCollapsed, setBusinessCollapsed] = useState(
    isEditing ? true : !!(settings.businessName && settings.businessEmail)
  );
  ...
  const handleSave = useCallback(() => {
    ...
    validate, shakeError, isEditing, existingInvoice, invoiceNumber,
    issueDate, dueDate, currency, notes, businessName, businessEmail,
    businessPhone, businessAddress, clientName, clientEmail, lineItems,
    addInvoice, updateInvoice, router,
  ]);
  ...
  return (
    ...
    >
      ...
      >
        <View style={styles.invoiceMetaBanner}>
          ...
          <View style={styles.metaStatusBadge}>
            <Text style={styles.metaStatusText}>
              {isEditing
                ? (existingInvoice?.status ?? 'Draft').charAt(0).toUpperCase() + (existingInvoice?.status ?? 'draft').slice(1)
                : 'Draft'}
            </Text>
          </View>
        </View>
        ...
        <InvoiceSummaryCard subtotal={subtotal} currency={currency} />
